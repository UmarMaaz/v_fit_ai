...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-20">
      {/* Header */}
      <header className="sticky top-0 z-40 w-full bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-indigo-600 rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-200">
            <i className="fa-solid fa-shirt text-white"></i>
          </div>
          <div>
            <h1 className="font-bold text-xl tracking-tight">V-Fit <span className="text-indigo-600">AI</span></h1>
            <p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">Virtual Fitting Room</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-8">
          {results.length === 0 && !isProcessing ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {results.map((res) => (
                <div key={res.id} className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 overflow-hidden border border-slate-100 group">
                    ...
                    {res.status === 'pending' ? (
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <div className="relative">
                          ...
                          <div className="absolute inset-0 flex items-center justify-center">
                            <i className="fa-solid fa-scissors text-indigo-400 animate-bounce"></i>
                          </div>
                        </div>
                        <p className="mt-4 text-sm font-bold text-indigo-600 animate-pulse">Tailoring {res.garmentName}...</p>
                        <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-widest font-bold">Anchoring Subject</p>
                      </div>
                    ) : res.status === 'error' ? (
                      <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center">
                        <i className="fa-solid fa-triangle-exclamation text-4xl text-red-300 mb-4"></i>
                        <p className="text-sm font-bold text-slate-800">Something went wrong</p>
                        <p className="text-xs text-slate-500 mt-2">{res.errorMessage}</p>
                        <button 
                          onClick={handleGenerate}
                          className="mt-6 text-xs font-bold text-indigo-600 uppercase tracking-widest hover:text-indigo-700"
                        >
                          Retry Generation
                        </button>
                      </div>
                      ...
                      <>
                        ...
                        <div className="absolute top-4 left-4 bg-white/90 backdrop-blur px-3 py-1.5 rounded-full shadow-sm">
                          <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-tighter">AI Fitting • {fitStyle}</p>
                        </div>
                        <div className="absolute bottom-0 inset-x-0 p-6 bg-gradient-to-t from-black/60 to-transparent translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all">
                          <div className="flex justify-between items-center">
                            <p className="text-white text-sm font-semibold truncate pr-4">{res.garmentName}</p>
                            <a 
                              href={res.imageUrl} 
                              download={`vfit-${res.id}.png`}
                              className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-900 hover:bg-indigo-600 hover:text-white transition-colors"
                            >
              ...
              {isProcessing && garments.length > results.length && (
                <div className="bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200 flex items-center justify-center h-full min-h-[400px]">
                  <p className="text-slate-400 font-medium">Queueing next items...</p>
                </div>
              )}
            </div>
          )}
